...
public sealed partial class TrainingsTab : IDisposable
{
    ...
    private async Task OnSelectionChanged(IEnumerable<DrogeUser> selection)
    {
        ...
        _reportTrainings = await ReportTrainingRepository.GetLastTraining(selection, 10, 0, _cls.Token);
        _currentPage = 1;
        _busy = false;
        StateHasChanged();
    }
    ...
    private async Task Next(int nextPage)
    {
        ...
        _reportTrainings = await ReportTrainingRepository.GetLastTraining(_selectedUsersTraining, _count, skip, _cls.Token);
        _busy = false;
        StateHasChanged();
    }
