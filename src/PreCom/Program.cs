    ...
    while (true)
    {
        try
        {
            ...
            if (nextRunMode != NextRunMode.None)
            {
                ...
                {
                    //using HttpClient WhatsAppclient = await SendWithWhatsapp(message);
                }
            }
            else
            {
...
static async Task<HttpClient> SendWithWhatsapp(string message)
{
    //However, will never work for groups like this :'(
    HttpClient WhatsAppclient = new();
